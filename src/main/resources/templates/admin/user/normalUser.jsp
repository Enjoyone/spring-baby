...
<link rel="stylesheet" href="admin/user/normalUser.css">

</head>
<body>
<div id="back" disabled="disabled"></div>   <!-- 虚化层 -->
				...
				<div class="con-next">
					...
					<div class="table-responsive ">
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								<c:forEach items="${normalUserList}" var="p" varStatus="vs">
									<tr>
										<td></td>
										<td>${vs.count }</td>
										<td><a href="javascript:void(0)" class="showThis">${p.userID}</a><a
											href="userIndex?userID=${p.userID}"
											class="badge badge-pill badge-info">个人首页</a></td>
										<td>${p.userName}</td>
										...
										<td>${p.email }</td>
										<td>${p.weChat}</td>
										<td class="introduction">${fn:substring(p.introduction,0,10)}<ins></ins><span>${p.introduction}</span></td>
							...
							<c:forEach items="${normalUserList}" var="p" varStatus="vs">
								<tbody class="${p.userID}">
									<tr>
										...
										</a> <a href="userIndex?userID=${p.userID}"
											class="badge badge-pill badge-info">个人首页 </a></td>
									</tr>
									<tr>
										<td>姓名：</td>
									...
									<tr>
										...
										<td>${p.weChat}</td>
										<td rowspan="3">介绍：</td>
										<td rowspan="3">${p.introduction}</td>
									</tr>
									<tr>
										<td>手机：</td>
										<td>${p.phone}</td>
